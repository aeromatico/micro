<?php
/**
 * Cuerpo de la galería de componentes — compartido entre
 * ComponentGallery::index() y la pestaña "Componentes" de ContentEditor.
 * Espera $blocks, $themes, $defaultTheme, $previewUrl y $firstBlock ya
 * definidos por el controlador que lo incluye.
 */
?>
...
<div class="padded-container" x-data="componentGallery()" x-init="init()">
    ...
    <!-- ==================================================================
         CONTROLES GLOBALES
         ================================================================== -->
    <div class="form-inline" style="display:flex; gap:24px; align-items:flex-end; flex-wrap:wrap; margin-bottom:20px;">
        <div>
            <label class="form-label" style="display:block; margin-bottom:4px;">Tema visual</label>
        ...
        <div>
            <label class="form-label" style="display:block; margin-bottom:4px;">Modo</label>
            <div class="btn-group">
        ...
        <div>
            <label class="form-label" style="display:block; margin-bottom:4px;">Ancho de vista</label>
            <div class="btn-group">
    ...
    <div x-show="activeBlock === '<?= e($key) ?>'" x-cloak>
        ...
        <div style="display:grid; grid-template-columns:repeat(<?= (int) $block['columns'] ?>, minmax(0, 1fr)); gap:20px; margin-bottom:40px;">
            ...
            <div class="callout fade in no-subheader" style="padding:0; overflow:hidden;" x-data="{ codeOpen: false, copied: false }">
                <div style="padding:10px 15px; border-bottom:1px solid rgba(127,127,127,.2); display:flex; justify-content:space-between; align-items:center; gap:8px;">
                    <strong><?= e($label) ?></strong>
                    <code class="text-muted" style="font-size:11px;"><?= e($value) ?></code>
                </div>
                ...
                <div style="padding:16px; display:flex; justify-content:center;">
                    <div :style="frameWrapperStyle()">
                        <iframe
                            ...
                            style="width:100%; height:<?= (int) $block['previewHeight'] ?>px; border:0; border-radius:4px;"
                            loading="lazy"
                        ></iframe>
                    </div>
                </div>

                <div style="border-top:1px solid rgba(127,127,127,.2);">
                    ...
                    <div x-show="codeOpen" style="padding:0 15px 15px;">
                        <div style="position:relative;">
                            ...
                            <button type="button" class="btn btn-default btn-sm" style="position:absolute; top:8px; right:8px;"
                                ...
                                <span x-show="copied" class="text-success"><i class="icon-check"></i> Copiado</span>
                            </button>
                        </div>
                    </div>
                </div>
